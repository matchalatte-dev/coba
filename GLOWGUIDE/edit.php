...
// update data di database {where id mengubah data yang dipilih}
    mysqli_query($conn,
...
// ambil data konsultasi berdasarkan id yang dikirim lewat URL
$id   = (int) $_GET['id'];
$data = mysqli_query($conn, "SELECT * FROM konsultasi WHERE id='$id'");
$d    = mysqli_fetch_array($data);
// jika data tidak ditemukan, redirect ke halaman konsultasi
if (!$d) {
    header("Location: konsultasi.php");
    exit();
}
...
<head>
    ...
    <title>Edit Konsultasi – GLOWGUIDE</title>
...
<body>
    ...
    <h1 class="page-title">✏️ Edit Konsultasi</h1>
    <p class="page-sub">Ubah data konsultasi <strong><?= htmlspecialchars($d['nama']) ?></strong></p>
        ...
        <div class="form-wrapper">
            ...
            <div class="card">
                ...
                <div class="row">
                    <div class="field">
                        <label>Umur</label>
                        <input type="number" name="umur" value="<?= $d['umur'] ?>" min="10" max="99" required>
                    </div>
                    <div class="field">
                ...
                <div class="row">
                    ...
                    <div class="field">
                        <label>Profesi</label>
                        <input type="text" name="profesi" value="<?= htmlspecialchars($d['profesi'] ?? '') ?>">
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <div class="field">
                    ...
                    <select name="jenis_kulit" required>
                        <?php
                        $options = ['Kering', 'Berminyak', 'Kombinasi', 'Sensitif', 'Normal'];
                        foreach ($options as $opt):
                        ?>
                            <option value="<?= $opt ?>" <?= $d['jenis_kulit'] == $opt ? 'selected' : '' ?>>
                                Kulit <?= $opt ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="field">
                    ...
                    <textarea name="keluhan" required><?= htmlspecialchars($d['keluhan']) ?></textarea>
                </div>
            </div>
